...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"darkpaycoinprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"darkpaycoinprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue importaddress(const UniValue& params, bool fHelp)
{
    ...
    if (IsHex(params[0].get_str())) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid DarkPayCoin address or script");
    }
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"darkpaycoinaddress\"\n"
            "\nReveals the private key corresponding to 'darkpaycoinaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"darkpaycoinaddress\"   (string, required) The darkpaycoin address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "they will have FULL ACCESS to STEAL your DarkPayCoin. Giving this file to someone\n"
            "is the same thing as giving them all of the DarkPayCoin in your wallet! Never send\n"
            "this file to ANYONE that you do not trust with all of your DarkPayCoin!!!\n"
            "\nArguments:\n"
...
/*    LOCK2(cs_main, pwalletMain->cs_wallet);
    ...
    // produce output
    file << strprintf("# Wallet dump created by DarkPayCoin %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue dumpallprivatekeys(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet private key dump file created by DarkPayCoin %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
    file << strprintf("# * Created on %s\n", EncodeDumpTime(GetTime()));
    file << strprintf("# * Best block at time of backup was %i (%s),\n", chainActive.Height(), chainActive.Tip()->GetBlockHash().ToString());
    file << strprintf("#   mined on %s\n", EncodeDumpTime(chainActive.Tip()->GetBlockTime()));
    file << "\n";
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"darkpaycoinaddress\"\n"
            "\nEncrypts a private key corresponding to 'darkpaycoinaddress'.\n"
            "\nArguments:\n"
            "1. \"darkpaycoinaddress\"   (string, required) The darkpaycoin address for the private key (you must hold the key already)\n"
            "2. \"passdkpcase\"   (string, required) The passdkpcase you want the private key to be encrypted with - Valid special chars: !#$%&'()*+,-./:;<=>?`{|}~ \n"
            "\nResult:\n"
    ...
    string strAddress = params[0].get_str();
    string strPassdkpcase = params[1].get_str();
    ...
    string encryptedOut = BIP38_Encrypt(strAddress, strPassdkpcase, privKey, vchSecret.IsCompressed());
...
UniValue bip38decrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38decrypt \"darkpaycoinaddress\"\n"
            "\nDecrypts and then imports password protected private key.\n"
            "\nArguments:\n"
            "1. \"encryptedkey\"   (string, required) The encrypted private key\n"
            "2. \"passdkpcase\"   (string, required) The passdkpcase you want the private key to be encrypted with\n"
    ...
    /** Collect private key and passdkpcase **/
    string strKey = params[0].get_str();
    string strPassdkpcase = params[1].get_str();

    uint256 privKey;
    bool fCompressed;
    if (!BIP38_Decrypt(strPassdkpcase, strKey, privKey, fCompressed))
