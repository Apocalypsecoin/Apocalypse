...
#if defined(NDEBUG)
#error "DarkPayCoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in udkpc) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock) {
    //Check to see if the zDKPC is properly signed
    if (pindex->nHeight > Params().Zerocoin_LastOldParams()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zDKPC spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zDKPC without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zDKPC spend with serial %s is already in block %d\n", __func__,
    ...
    if (!spend.HasValidSerial(paramsToUse))
        return error("%s : zDKPC spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zDKPC spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zdkpc");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zDKPC mints are not already known
            if (tx.IsZerocoinMint()) {
                for (auto& out : tx.vout) {
                    if (!out.IsZerocoinMint())
                        continue;
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
       nSubsidy = 21000 * COIN; //premine for first MNs
    }
    else if (nHeight < 267 && nHeight > 0) { // testing phase
        nSubsidy = 6.66 * COIN;
    }
    else if (nHeight < 20000 && nHeight > 266) { // warming phase
        nSubsidy = 4 * COIN;
    }
    else if (nHeight <= 29999 && nHeight >= 20000) { // GO !!
        nSubsidy = 40 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from DarkPayCoin
...
{
    RenameThread("darkpaycoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZDKPCMinted()
{
...
void RecalculateZDKPCSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zDKPC supply
...
bool RecalculateDKPCSupply(int nHeightStart)
{
...
bool UpdateZDKPCSupply(const CBlock& block, CBlockIndex* pindex)
{
    std::list<CZerocoinMint> listMints;
    BlockToZerocoinMintList(block, listMints);
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    UpdateZDKPCSupply(block, pindex);
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zDKPCSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert DKPC to zDKPC
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if (nHeight > Params().Zerocoin_StartHeight()) {
        ...
        for (const CTransaction& tx : block.vtx) {
            ...
            // double check that there are no double spent zDKPC spends in this block
            if (tx.IsZerocoinSpend()) {
                for (const CTxIn txIn : tx.vin) {
                    if (txIn.scriptSig.IsZerocoinSpend()) {
                        libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                        if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                            return state.DoS(100, error("%s : Double spending of zDKPC serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zDKPC mints and %d zDKPC spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        if (pfrom->cleanSubVer == "/DarkPayCoin Core:1.1.0/" ||
            pfrom->cleanSubVer == "/DarkPayCoin Core:1.3.0/" ||
            pfrom->cleanSubVer == "/DarkPayCoin Core:1.3.1/") {
            LOCK(cs_main);
        ...
        // DarkPayCoin: We use certain sporks during IBD, so check to see if they are
