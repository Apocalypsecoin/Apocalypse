...
void ComputePreFactor(std::string strPassdkpcase, std::string strSalt, uint256& prefactor)
{
    //passfactor is the scrypt hash of passdkpcase and ownersalt (NOTE this needs to handle alt cases too in the future)
    uint64_t s = uint256(ReverseEndianString(strSalt)).Get64();
    scrypt_hash(strPassdkpcase.c_str(), strPassdkpcase.size(), BEGIN(s), strSalt.size() / 2, BEGIN(prefactor), 16384, 8, 8, 32);
}
...
std::string BIP38_Encrypt(std::string strAddress, std::string strPassdkpcase, uint256 privKey, bool fCompressed)
{
    string strAddressHash = AddressToBip38Hash(strAddress);

    uint512 hashed;
    uint64_t salt = uint256(ReverseEndianString(strAddressHash)).Get64();
    scrypt_hash(strPassdkpcase.c_str(), strPassdkpcase.size(), BEGIN(salt), strAddressHash.size() / 2, BEGIN(hashed), 16384, 8, 8, 64);
...
bool BIP38_Decrypt(std::string strPassdkpcase, std::string strKey, uint256& privKey, bool& fCompressed)
{
    ...
    uint256 prefactor;
    ComputePreFactor(strPassdkpcase, prefactorSalt, prefactor);
