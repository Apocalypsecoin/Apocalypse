...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zdkpcTracker = unique_ptr<CzDKPCTracker>(new CzDKPCTracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == ZDKPC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == ZDKPC_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
