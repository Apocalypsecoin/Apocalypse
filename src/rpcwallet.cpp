...
std::string HelpRequiringPassdkpcase()
{
    return pwalletMain && pwalletMain->IsCrypted() ? "\nRequires wallet passdkpcase to be set with walletpassdkpcase call." : "";
}
...
void EnsureWalletIsUnlocked(bool fAllowAnonOnly)
{
    if (pwalletMain->IsLocked() || (!fAllowAnonOnly && pwalletMain->fWalletUnlockAnonymizeOnly))
        throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED, "Error: Please enter the wallet passdkpcase with walletpassdkpcase first.");
}
...
UniValue getnewaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "\nReturns a new DarkPayCoin address for receiving payments.\n"
            ...
            "\nResult:\n"
            "\"darkpaycoinaddress\"    (string) The new darkpaycoin address\n"
            "\nExamples:\n" +
...
UniValue getaccountaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns the current DarkPayCoin address for receiving payments to this account.\n"
            "\nArguments:\n"
            "1. \"account\"       (string, required) The account name for the address. It can also be set to the empty string \"\" to represent the default account. The account does not need to exist, it will be created and a new address created  if there is no account by the given name.\n"
            "\nResult:\n"
            "\"darkpaycoinaddress\"   (string) The account darkpaycoin address\n"
            "\nExamples:\n" +
...
UniValue getrawchangeaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "\nReturns a new DarkPayCoin address, for receiving change.\n"
...
UniValue setaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"darkpaycoinaddress\" \"account\"\n"
            "\nSets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"darkpaycoinaddress\"  (string, required) The darkpaycoin address to be associated with an account.\n"
    ...
    if (!IsValidDestinationString(params[0].get_str()))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid DarkPayCoin address");
...
UniValue getaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "getaccount \"darkpaycoinaddress\"\n"
            "\nReturns the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"darkpaycoinaddress\"  (string, required) The darkpaycoin address for account lookup.\n"
            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "  \"darkpaycoinaddress\"  (string) a darkpaycoin address associated with the given account\n"
            "  ,...\n"
            "]\n"
            "\nExamples:\n" +
...
UniValue sendtoaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress \"darkpaycoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassdkpcase() +
            "\nArguments:\n"
            "1. \"darkpaycoinaddress\"  (string, required) The darkpaycoin address to send to.\n"
            "2. \"amount\"      (numeric, required) The amount in darkpaycoin to send. e.g. 0.1\n"
...
UniValue sendtoaddressix(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddressix \"darkpaycoinaddress\" amount ( \"comment\" \"comment-to\" )\n"
            "\nSend an amount to a given address. The amount is a real and is rounded to the nearest 0.00000001\n" +
            HelpRequiringPassdkpcase() +
            "\nArguments:\n"
            "1. \"darkpaycoinaddress\"  (string, required) The darkpaycoin address to send to.\n"
...
UniValue listaddressgroupings(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "  [\n"
            "    [\n"
            "      \"darkpaycoinaddress\",     (string) The darkpaycoin address\n"
            "      amount,                 (numeric) The amount in darkpaycoin\n"
...
UniValue signmessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"darkpaycoinaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassdkpcase() + "\n"
                                        "\nArguments:\n"
                                        "1. \"darkpaycoinaddress\"  (string, required) The darkpaycoin address to use for the private key.\n"
                                        "2. \"message\"         (string, required) The message to create a signature of.\n"
                                        "\nResult:\n"
                                        "\"signature\"          (string) The signature of the message encoded in base 64\n"
                                        "\nExamples:\n"
                                        "\nUnlock the wallet for 30 seconds\n" +
            HelpExampleCli("walletpassdkpcase", "\"mypassdkpcase\" 30") +
...
UniValue getreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress \"darkpaycoinaddress\" ( minconf )\n"
            "\nReturns the total amount received by the given darkpaycoinaddress in transactions with at least minconf confirmations.\n"
            "\nArguments:\n"
            "1. \"darkpaycoinaddress\"  (string, required) The darkpaycoin address for transactions.\n"
            "2. minconf             (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount   (numeric) The total amount in darkpaycoin received at this address.\n"
            "\nExamples:\n"
...
UniValue getreceivedbyaccount(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "2. minconf          (numeric, optional, default=1) Only include transactions confirmed at least this many times.\n"
            "\nResult:\n"
            "amount              (numeric) The total amount in darkpaycoin received for this account.\n"
            "\nExamples:\n"
...
UniValue movecmd(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 5)
        throw runtime_error(
            ...
            "\nExamples:\n"
            "\nMove 0.01 darkpaycoin from the default account to the account named tabby\n" +
            HelpExampleCli("move", "\"\" \"tabby\" 0.01") +
            "\nMove 0.01 darkpaycoin timotei to akiko with a comment and funds have 6 confirmations\n" + HelpExampleCli("move", "\"timotei\" \"akiko\" 0.01 6 \"happy birthday!\"") +
...
UniValue sendfrom(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"todarkpaycoinaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nSent an amount from an account to a darkpaycoin address.\n"
            "The amount is a real and is rounded to the nearest 0.00000001." +
            HelpRequiringPassdkpcase() + "\n"
                                        "\nArguments:\n"
                                        "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the default account using \"\".\n"
                                        "2. \"todarkpaycoinaddress\"  (string, required) The darkpaycoin address to send funds to.\n"
                                        "3. amount                (numeric, required) The amount in darkpaycoin. (transaction fee is added on top).\n"
                                        ...
                                        "\"transactionid\"        (string) The transaction id.\n"
                                        "\nExamples:\n"
                                        "\nSend 0.01 darkpaycoin from the default account to the address, must have at least 1 confirmation\n" +
...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            HelpRequiringPassdkpcase() + "\n"
                                        ...
                                        "    {\n"
                                        "      \"address\":amount   (numeric) The darkpaycoin address is the key, the numeric amount in darkpaycoin is the value\n"
                                        "      ,...\n"
                                        "    }\n"
    ...
    BOOST_FOREACH(const string& name_, keys) {
        if (!IsValidDestinationString(name_))
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid DarkPayCoin address: ")+name_);
...
UniValue addmultisigaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3) {
        string msg = "addmultisigaddress nrequired [\"key\",...] ( \"account\" )\n"
                     ...
                     "Each key is a DarkPayCoin address or hex-encoded public key.\n"
                     "If 'account' is specified, assign address to that account.\n"

                     "\nArguments:\n"
                     "1. nrequired        (numeric, required) The number of required signatures out of the n keys or addresses.\n"
                     "2. \"keysobject\"   (string, required) A json array of darkpaycoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"address\"  (string) darkpaycoin address or hex-encoded public key\n"
                     "       ...,\n"
                     "     ]\n"
                     "3. \"account\"      (string, optional) An account to assign the addresses to.\n"

                     "\nResult:\n"
                     "\"darkpaycoinaddress\"  (string) A darkpaycoin address associated with the keys.\n"
...
UniValue listreceivedbyaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"amount\" : x.xxx,                  (numeric) The total amount in darkpaycoin received by the address\n"
...
UniValue listtransactions(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"darkpaycoinaddress\",    (string) The darkpaycoin address of the transaction. Not present for \n"
            "                                                move transactions (category = move).\n"
            "    \"category\":\"send|receive|move\", (string) The transaction category. 'move' is a local (off blockchain)\n"
            "                                                transaction between accounts, and not associated with an address,\n"
            "                                                transaction id or block. 'send' and 'receive' transactions are \n"
            "                                                associated with an address, transaction id and block details\n"
            "    \"amount\": x.xxx,          (numeric) The amount in darkpaycoin. This is negative for the 'send' category, and for the\n"
            "                                         'move' category for moves outbound. It is positive for the 'receive' category,\n"
            "                                         and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in darkpaycoin. This is negative and only available for the \n"
...
UniValue listsinceblock(const UniValue& params, bool fHelp)
{
    if (fHelp)
        throw runtime_error(
            ...
            "    \"address\":\"darkpaycoinaddress\",    (string) The darkpaycoin address of the transaction. Not present for move transactions (category = move).\n"
            "    \"category\":\"send|receive\",     (string) The transaction category. 'send' has negative amounts, 'receive' has positive amounts.\n"
            "    \"amount\": x.xxx,          (numeric) The amount in darkpaycoin. This is negative for the 'send' category, and for the 'move' category for moves \n"
            "                                          outbound. It is positive for the 'receive' category, and for the 'move' category for inbound funds.\n"
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"fee\": x.xxx,             (numeric) The amount of the fee in darkpaycoin. This is negative and only available for the 'send' category of transactions.\n"
...
UniValue gettransaction(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"amount\" : x.xxx,        (numeric) The transaction amount in darkpaycoin\n"
            ...
            "      \"address\" : \"darkpaycoinaddress\",   (string) The darkpaycoin address involved in the transaction\n"
            "      \"category\" : \"send|receive\",    (string) The category, either 'send' or 'receive'\n"
            "      \"amount\" : x.xxx                  (numeric) The amount in darkpaycoin\n"
            "      \"vout\" : n,                       (numeric) the vout value\n"
            "    }\n"
            "    ,...\n"
            "  ],\n"
...
UniValue walletpassdkpcase(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() < 2 || params.size() > 3))
        throw runtime_error(
            "walletpassdkpcase \"passdkpcase\" timeout ( anonymizeonly )\n"
            "\nStores the wallet decryption key in memory for 'timeout' seconds.\n"
            "This is needed prior to performing transactions related to private keys such as sending DKPCs\n"
            "\nArguments:\n"
            "1. \"passdkpcase\"     (string, required) The wallet passdkpcase\n"
            "2. timeout            (numeric, required) The time to keep the decryption key in seconds.\n"
            "3. anonymizeonly      (boolean, optional, default=flase) If is true sending functions are disabled."
            "\nNote:\n"
            "Issuing the walletpassdkpcase command while the wallet is already unlocked will set a new unlock\n"
            "time that overrides the old one. A timeout of \"0\" unlocks until the wallet is closed.\n"
            "\nExamples:\n"
            "\nUnlock the wallet for 60 seconds\n" +
            HelpExampleCli("walletpassdkpcase", "\"my pass dkpcase\" 60") +
            "\nUnlock the wallet for 60 seconds but allow anonymization, automint, and staking only\n" + HelpExampleCli("walletpassdkpcase", "\"my pass dkpcase\" 60 true") +
            "\nLock the wallet again (before 60 seconds)\n" + HelpExampleCli("walletlock", "") +
            "\nAs json rpc call\n" + HelpExampleRpc("walletpassdkpcase", "\"my pass dkpcase\", 60"));
    ...
    if (!pwalletMain->IsCrypted())
        throw JSONRPCError(RPC_WALLET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletpassdkpcase was called.");

    // Note that the walletpassdkpcase is stored in params[0] which is not mlock()ed
    ...
    if (!pwalletMain->Unlock(strWalletPass, anonymizeOnly))
        throw JSONRPCError(RPC_WALLET_PASSDKPCASE_INCORRECT, "Error: The wallet passdkpcase entered was incorrect.");
...
UniValue walletpassdkpcasechange(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
        throw runtime_error(
            "walletpassdkpcasechange \"oldpassdkpcase\" \"newpassdkpcase\"\n"
            "\nChanges the wallet passdkpcase from 'oldpassdkpcase' to 'newpassdkpcase'.\n"
            "\nArguments:\n"
            "1. \"oldpassdkpcase\"      (string) The current passdkpcase\n"
            "2. \"newpassdkpcase\"      (string) The new passdkpcase\n"
            "\nExamples:\n" +
            HelpExampleCli("walletpassdkpcasechange", "\"old one\" \"new one\"") + HelpExampleRpc("walletpassdkpcasechange", "\"old one\", \"new one\""));
    ...
    if (fHelp)
        return true;
    if (!pwalletMain->IsCrypted())
        throw JSONRPCError(RPC_WALLET_WRONG_ENC_STATE, "Error: running with an unencrypted wallet, but walletpassdkpcasechange was called.");
    ...
    if (!pwalletMain->ChangeWalletPassdkpcase(strOldWalletPass, strNewWalletPass))
        throw JSONRPCError(RPC_WALLET_PASSDKPCASE_INCORRECT, "Error: The wallet passdkpcase entered was incorrect.");
...
UniValue walletlock(const UniValue& params, bool fHelp)
{
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 0))
        throw runtime_error(
            ...
            "After calling this method, you will need to call walletpassdkpcase again\n"
            "before being able to call any methods which require the wallet to be unlocked.\n"
            "\nExamples:\n"
            "\nSet the passdkpcase for 2 minutes to perform a transaction\n" +
            HelpExampleCli("walletpassdkpcase", "\"my pass dkpcase\" 120") +
            "\nPerform a send (requires passdkpcase set)\n" + HelpExampleCli("sendtoaddress", "\"XwnLY9Tf7Zsef8gMGL2fhWA9ZmMjt4KPwg\" 1.0") +
            "\nClear the passdkpcase since we are done before 2 minutes is up\n" + HelpExampleCli("walletlock", "") +
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            "encryptwallet \"passdkpcase\"\n"
            "\nEncrypts the wallet with 'passdkpcase'. This is for first time encryption.\n"
            "After this, any calls that interact with private keys such as sending or signing \n"
            "will require the passdkpcase to be set prior the making these calls.\n"
            "Use the walletpassdkpcase call for this, and then walletlock call.\n"
            "If the wallet is already encrypted, use the walletpassdkpcasechange call.\n"
            "Note that this will shutdown the server.\n"
            "\nArguments:\n"
            "1. \"passdkpcase\"    (string) The pass dkpcase to encrypt the wallet with. It must be at least 1 character, but should be long.\n"
            "\nExamples:\n"
            "\nEncrypt you wallet\n" +
            HelpExampleCli("encryptwallet", "\"my pass dkpcase\"") +
            "\nNow set the passdkpcase to use the wallet, such as for signing or sending DKPCs\n" + HelpExampleCli("walletpassdkpcase", "\"my pass dkpcase\"") +
            "\nNow we can so something like sign\n" + HelpExampleCli("signmessage", "\"darkpaycoinaddress\" \"test message\"") +
            "\nNow lock the wallet again by removing the passdkpcase\n" + HelpExampleCli("walletlock", "") +
            "\nAs a json rpc call\n" + HelpExampleRpc("encryptwallet", "\"my pass dkpcase\""));
    ...
    if (strWalletPass.length() < 1)
        throw runtime_error(
            "encryptwallet <passdkpcase>\n"
            "Encrypts the wallet with <passdkpcase>.");
    ...
    StartShutdown();
    return "wallet encrypted; darkpaycoin server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue lockunspent(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending DKPCs.\n"
...
UniValue settxfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount         (numeric, required) The transaction fee in DKPC/kB rounded to the nearest 0.00000001\n"
            "\nResult\n"
...
UniValue getwalletinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total DKPC balance of the wallet\n"
...
UniValue autocombinerewards(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || (fEnable && params.size() != 2) || params.size() > 2)
        throw runtime_error(
            ...
            "\nWallet will automatically monitor for any coins with value below the threshold amount, and combine them if they reside with the same DarkPayCoin address\n"
...
UniValue multisend(const UniValue& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "multisend <DarkPayCoin Address> <percent>\n"
    ...
    string strAddress = params[0].get_str();
    if (!IsValidDestinationString(strAddress))
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid DKPC address");
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    set<CMintMeta> setMints = pwalletMain->zdkpcTracker->ListMints(true, true, true);
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "mintzerocoin <amount>\n"
            "Usage: Enter an amount of Dkpc to convert to zDkpc"
            + HelpRequiringPassdkpcase());
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE))
        throw JSONRPCError(RPC_WALLET_ERROR, "zDKPC is currently disabled due to maintenance.");
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 5 || params.size() < 4)
        throw runtime_error(
            ...
            "Overview: Convert zDKPC (zerocoins) into DKPC. \n"
            "amount: amount to spend\n"
            "mintchange: if there is left over DKPC (change), the wallet can convert it automatically back to zerocoins [true]\n"
    ...
    bool fMintChange = params[1].get_bool();        // Mint change to zDKPC
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CzDKPCTracker* zdkpcTracker = pwalletMain->zdkpcTracker.get();
    set<CMintMeta> setMints = zdkpcTracker->ListMints(false, false, true);
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzDKPCTracker* zdkpcTracker = pwalletMain->zdkpcTracker.get();
    set<CMintMeta> setMints = zdkpcTracker->ListMints(false, false, false);
    ...
    for (CZerocoinSpend spend : listUnconfirmedSpends) {
        for (auto& meta : setMints) {
            if (meta.hashSerial == GetSerialHash(spend.GetSerial())) {
                zdkpcTracker->SetPubcoinNotUsed(meta.hashPubcoin);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.empty() || params.size() > 2)
        throw runtime_error(
            "exportzerocoins include_spent ( denomination )\n"
                ...
                "2. \"denomination\"         (integer, optional) Export a specific denomination of zDkpc\n"
    ...
    CzDKPCTracker* zdkpcTracker = pwalletMain->zdkpcTracker.get();
    set<CMintMeta> setMints = zdkpcTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue importzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() == 0)
        throw runtime_error(
            "importzerocoins importdata \n"
                ...
                "\"value\"            (string) the total zDkpc value of zerocoin mints that were added\n"
    ...
    for (unsigned int idx = 0; idx < arrMints.size(); idx++) {
        ...
        pwalletMain->zdkpcTracker->Add(mint, true);
        count++;
...
UniValue reconsiderzerocoins(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "reconsiderzerocoins\n"
                "\nCheck archived zDkpc list to see if any mints were added to the blockchain.\n"
...
UniValue setzdkpcseed(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 1)
        throw runtime_error(
            "setzdkpcseed \"seed\"\n"
            "\nSet the wallet's deterministic zdkpc seed to a specific value.\n" +
            HelpRequiringPassdkpcase() + "\n"

            "\nArguments:\n"
            "1. \"seed\"        (string, required) The deterministic zdkpc seed.\n"

            "\nResult\n"
            "\"success\" : b,  (boolean) Whether the seed was successfully set.\n"

            "\nExamples\n" +
            HelpExampleCli("setzdkpcseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5") +
            HelpExampleRpc("setzdkpcseed", "63f793e7895dd30d99187b35fbfb314a5f91af0add9e0a4e5877036d1e392dd5"));
    ...
    CzDKPCWallet* zwallet = pwalletMain->getZWallet();
...
UniValue getzdkpcseed(const UniValue& params, bool fHelp)
{
    if(fHelp || !params.empty())
        throw runtime_error(
            "getzdkpcseed\n"
            ...
            "\nResult\n"
            "\"seed\" : s,  (string) The deterministic zDKPC seed.\n"

            "\nExamples\n" +
            HelpExampleCli("getzdkpcseed", "") + HelpExampleRpc("getzdkpcseed", ""));
...
UniValue generatemintlist(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 2)
        throw runtime_error(
            "generatemintlist\n"
            "\nShow mints that are derived from the deterministic zDKPC seed.\n" +
            HelpRequiringPassdkpcase() + "\n"

            "\nArguments\n"
            "1. \"count\"  : n,  (numeric) Which sequential zDKPC to start with.\n"
            "2. \"range\"  : n,  (numeric) How many zDKPC to generate.\n"
    ...
    CzDKPCWallet* zwallet = pwalletMain->zwalletMain;
...
UniValue dzdkpcstate(const UniValue& params, bool fHelp) {
    if (fHelp || params.size() != 0)
        throw runtime_error(
                "dzdkpcstate\n"
                        "\nThe current state of the mintpool of the deterministic zDKPC wallet.\n" +
                HelpRequiringPassdkpcase() + "\n"
    ...
    obj.push_back(Pair("dzdkpc_count", nCount));
...
void static SearchThread(CzDKPCWallet* zwallet, int nCountStart, int nCountEnd)
{
    ...
    try {
        ...
        for(int i = nCountStart; i < nCountEnd; i++) {
            ...
            CKey key;
            zwallet->SeedToZDKPC(zerocoinSeed, bnValue, bnSerial, bnRandomness, key);
...
UniValue searchdzdkpc(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 3)
        throw runtime_error(
            "searchdzdkpc\n"
            "\nMake an extended search for deterministically generated zDKPC that have not yet been recognized by the wallet.\n" +
            HelpRequiringPassdkpcase() + "\n"

            "\nArguments\n"
            "1. \"count\"       (numeric) Which sequential zDKPC to start with.\n"
            "2. \"range\"       (numeric) How many zDKPC to generate.\n"
            "3. \"threads\"     (numeric) How many threads should this operation consume.\n"

            "\nExamples\n" +
            HelpExampleCli("searchdzdkpc", "1, 100, 2") + HelpExampleRpc("searchdzdkpc", "1, 100, 2"));
    ...
    boost::thread_group* dzdkpcThreads = new boost::thread_group();
    ...
    for (int i = 0; i < nThreads; i++) {
        ...
        nPrevThreadEnd = nEnd;
        dzdkpcThreads->create_thread(boost::bind(&SearchThread, zwallet, nStart, nEnd));
    }

    dzdkpcThreads->join_all();

    zwallet->RemoveMintsFromPool(pwalletMain->zdkpcTracker->GetSerialHashes());
