...
CzDKPCWallet::CzDKPCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdkpc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDKPCSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDKPCSeed_deprecated()) {
                    LogPrintf("%s: Updated zDKPC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdkpc seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdkpc wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDKPCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDKPCCount(nCountLastUsed);
    else if (!walletdb.ReadZDKPCCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDKPCWallet::Lock()
{
    seedMaster = 0;
}

void CzDKPCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDKPCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDKPC(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDKPCWallet::LoadMintPoolFromDB()
{
...
void CzDKPCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDKPCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDKPCWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdkpcTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDKPCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDKPC(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdkpcTracker which also adds to database
    pwalletMain->zdkpcTracker->Add(dMint, true);
...
void CzDKPCWallet::SeedToZDKPC(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDKPCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDKPCWallet::GenerateDeterministicZDKPC(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDKPCWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDKPCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
