...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total darkpaycoin balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"zDKPCsupply\" :\n"
            "  {\n"
            "     \"1\" : n,            (numeric) supply of 1 zDKPC denomination\n"
            "     \"5\" : n,            (numeric) supply of 5 zDKPC denomination\n"
            "     \"10\" : n,           (numeric) supply of 10 zDKPC denomination\n"
            "     \"50\" : n,           (numeric) supply of 50 zDKPC denomination\n"
            "     \"100\" : n,          (numeric) supply of 100 zDKPC denomination\n"
            "     \"500\" : n,          (numeric) supply of 500 zDKPC denomination\n"
            "     \"1000\" : n,         (numeric) supply of 1000 zDKPC denomination\n"
            "     \"5000\" : n,         (numeric) supply of 5000 zDKPC denomination\n"
            "     \"total\" : n,        (numeric) The total supply of all zDKPC denominations\n"
            "  }\n"
            "  \"keypoololdest\": xxxxxx,    (numeric) the timestamp (seconds since GMT epoch) of the oldest pre-generated key in the key pool\n"
            "  \"keypoolsize\": xxxx,        (numeric) how many new keys are pre-generated\n"
            "  \"unlocked_until\": ttt,      (numeric) the timestamp in seconds since epoch (midnight Jan 1 1970 GMT) that the wallet is unlocked for transfers, or 0 if the wallet is locked\n"
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in darkpaycoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in darkpaycoin/kb\n"
    ...
    UniValue zdkpcObj(UniValue::VOBJ);
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zdkpcObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zdkpcObj.push_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.push_back(Pair("zDKPCsupply", zdkpcObj));
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are darkpaycoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) darkpaycoin address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"darkpaycoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"darkpaycoinaddress\"  (string, required) The darkpaycoin address to use for the signature.\n"
