...
#ifndef PHORE_ZDKPCWALLET_H
#define PHORE_ZDKPCWALLET_H
...
public:
    CzDKPCWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDKPC(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDKPC(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //PHORE_ZDKPCWALLET_H
