...
public:
    ...
    bool WriteZDKPCSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDKPCSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDKPCSeed_deprecated(uint256& seed);
    bool EraseZDKPCSeed();
    bool EraseZDKPCSeed_deprecated();

    bool WriteZDKPCCount(const uint32_t& nCount);
    bool ReadZDKPCCount(uint32_t& nCount);
