...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 DKPC at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zDKPC send
enum ZerocoinSpendStatus {
    ZDKPC_SPEND_OKAY = 0,                            // No error
    ZDKPC_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZDKPC_WALLET_LOCKED = 2,                         // Wallet was locked
    ZDKPC_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZDKPC_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZDKPC_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZDKPC_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZDKPC_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZDKPC_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZDKPC_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZDKPC_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZDKPC_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZDKPC_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZDKPC_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZDKPC_SPENT_USED_ZDKPC = 14,                      // Coin has already been spend
    ZDKPC_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZDKPC_SPEND_V1_SEC_LEVEL
};
...
public:
    ...
    void ZDkpcBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZDKPCOutput(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzdkpcAuto) const;
    ...
    */
    ...
    CzDKPCWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzDKPCTracker> zdkpcTracker;
    ...
    void setZWallet(CzDKPCWallet* zwallet)
    {
        zwalletMain = zwallet;
        zdkpcTracker = std::unique_ptr<CzDKPCTracker>(new CzDKPCTracker(strWalletFile));
    }

    CzDKPCWallet* getZWallet() { return zwalletMain; }
    ...
    void setZDkpcAutoBackups(bool fEnabled)
    {
    ...
    bool Unlock(const SecureString& strWalletPassdkpcase, bool anonimizeOnly = false);
    bool ChangeWalletPassdkpcase(const SecureString& strOldWalletPassdkpcase, const SecureString& strNewWalletPassdkpcase);
    bool EncryptWallet(const SecureString& strWalletPassdkpcase);
