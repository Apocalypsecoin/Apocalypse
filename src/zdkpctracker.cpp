...
CzDKPCTracker::CzDKPCTracker(std::string strWalletFile)
{
...
CzDKPCTracker::~CzDKPCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDKPCTracker::Init()
{
...
bool CzDKPCTracker::Archive(CMintMeta& meta)
{
...
bool CzDKPCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDKPCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDKPCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDKPCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzDKPCTracker::GetSerialHashes()
{
...
CAmount CzDKPCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDKPCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDKPCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDKPCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDKPCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDKPCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDKPCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDKPCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDKPCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDKPCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDKPCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDKPCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDKPCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDKPCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDKPCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDKPCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDKPCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzdkpc from DB\n", __func__, listDeterministicDB.size());
    }
