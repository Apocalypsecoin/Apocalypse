...
bool isValidPassdkpcase(QString strPassdkpcase, QString& strInvalid)
{
    for (int i = 0; i < strPassdkpcase.size(); i++) {
        if (!validChar.contains(strPassdkpcase[i], Qt::CaseSensitive)) {
            if (QString("\"'").contains(strPassdkpcase[i]))
                continue;

            strInvalid = strPassdkpcase[i];
            return false;
        }
    }
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    QString qstrPassdkpcase = ui->passdkpcaseIn_ENC->text();
    QString strInvalid;
    if (!isValidPassdkpcase(qstrPassdkpcase, strInvalid)) {
        ui->statusLabel_ENC->setStyleSheet("QLabel { color: red; }");
        ui->statusLabel_ENC->setText(tr("The entered passdkpcase is invalid. ") + strInvalid + QString(" is not valid") + QString(" ") + tr("Allowed: 0-9,a-z,A-Z,") + specialChar);
        return;
    }
    ...
    std::string encryptedKey = BIP38_Encrypt(EncodeDestination(addr), qstrPassdkpcase.toStdString(), key.GetPrivKey_256(), key.IsCompressed());
...
{
    string strPassdkpcase = ui->passdkpcaseIn_DEC->text().toStdString();
    ...
    if (!BIP38_Decrypt(strPassdkpcase, strKey, privKey, fCompressed)) {
        ...
        ui->statusLabel_DEC->setText(tr("Failed to decrypt.") + QString(" ") + tr("Please check the key and passdkpcase and try again."));
        return;
    }
