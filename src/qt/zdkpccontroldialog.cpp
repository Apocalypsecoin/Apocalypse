...
#include "ui_zdkpccontroldialog.h"
...
std::set<std::string> ZDkpcControlDialog::setSelectedMints;
std::set<CMintMeta> ZDkpcControlDialog::setMints;

ZDkpcControlDialog::ZDkpcControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDkpcControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZDkpcControlDialog::~ZDkpcControlDialog()
{
    delete ui;
}

void ZDkpcControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDkpcControlDialog::updateList()
{
...
void ZDkpcControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDkpcControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZDkpc_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDkpcControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZDkpcControlDialog::GetSelectedMints()
{
...
void ZDkpcControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
