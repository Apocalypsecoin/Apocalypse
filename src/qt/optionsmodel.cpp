...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DKPC);
    ...
    if (!settings.contains("nAnonymizeDarkPayCoinAmount"))
        settings.setValue("nAnonymizeDarkPayCoinAmount", 1000);

    nAnonymizeDarkPayCoinAmount = settings.value("nAnonymizeDarkPayCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDarkPayCoinAmount"))
        SoftSetArg("-anonymizedarkpaycoinamount", settings.value("nAnonymizeDarkPayCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in darkpaycoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDarkPayCoinAmount:
            return QVariant(nAnonymizeDarkPayCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDarkPayCoinAmount:
            nAnonymizeDarkPayCoinAmount = value.toInt();
            settings.setValue("nAnonymizeDarkPayCoinAmount", nAnonymizeDarkPayCoinAmount);
            emit anonymizeDarkPayCoinAmountChanged(nAnonymizeDarkPayCoinAmount);
            break;
        case CoinControlFeatures:
