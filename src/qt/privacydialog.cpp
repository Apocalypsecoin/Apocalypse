                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zDKPC ought to be enough for anybody." - Bill Gates, 2017
    ui->zDKPCpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzDKPCSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zDKPCpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzDKPC_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDKPC is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zDKPC...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zDKPC in ") +
...
void PrivacyDialog::on_pushButtonSpendzDKPC_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDKPC is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zDKPC
        sendzDKPC();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zDKPC
    sendzDKPC();
}

void PrivacyDialog::on_pushButtonZDkpcControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ZDkpcControlDialog* zDkpcControl = new ZDkpcControlDialog(this);
    zDkpcControl->setModel(walletModel);
    zDkpcControl->exec();
}

void PrivacyDialog::setZDkpcControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDKPCSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDKPC()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!IsValidDestinationString(ui->payTo->text().toStdString())) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid DarkPayCoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zDKPCpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zDKPC is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DKPC </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zDKPC</b>";
    ...
    // use mints from zDKPC selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!ZDkpcControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZDkpcControlDialog::GetSelectedMints();
        ...
        for (auto& meta : vMintsToFetch) {
            if (meta.nVersion < libzerocoin::PrivateCoin::PUBKEY_VERSION) {
                ...
                if (nSecurityLevel < 100) {
                    QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zDKPC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
                    ui->TEMintStatus->setPlainText(tr("Failed to spend zDKPC"));
                    ui->TEMintStatus->repaint();
                    return;
                }
            }
    ...
    if (!fSuccess) {
        if (receipt.GetStatus() == ZDKPC_SPEND_V1_SEC_LEVEL) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Version 1 zDKPC require a security level of 100 to successfully spend."), QMessageBox::Ok, QMessageBox::Ok);
            ui->TEMintStatus->setPlainText(tr("Failed to spend zDKPC"));
            ui->TEMintStatus->repaint();
            return;
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zDKPC was spent successfully update the addressbook with the label
    ...
    // Clear zdkpc selector in case it was used
    ZDkpcControlDialog::setSelectedMints.clear();
    ui->labelzDKPCSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDKPC Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Dkpc, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zDKPC Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zdkpcTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDKPC </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zDKPC "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zDKPC "));
    ui->labelzDKPCAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzDKPCSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzDKPC->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zDKPC
        ui->pushButtonMintzDKPC->setEnabled(false);
        ui->pushButtonMintzDKPC->setToolTip(tr("zDKPC is currently disabled due to maintenance."));

        // Spend zDKPC
        ui->pushButtonSpendzDKPC->setEnabled(false);
        ui->pushButtonSpendzDKPC->setToolTip(tr("zDKPC is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zDKPC
        ui->pushButtonMintzDKPC->setEnabled(true);
        ui->pushButtonMintzDKPC->setToolTip(tr("PrivacyDialog", "Enter an amount of DKPC to convert to zDKPC", 0));

        // Spend zDKPC
        ui->pushButtonSpendzDKPC->setEnabled(true);
        ui->pushButtonSpendzDKPC->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}