...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered darkpaycoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 DKPC.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal darkpaycoin:URI (darkpaycoin:XyZ...?message=example)
            {
...
bool WalletModel::setWalletEncrypted(bool encrypted, const SecureString& passdkpcase)
{
    if (encrypted) {
        // Encrypt
        return wallet->EncryptWallet(passdkpcase);
    } else {
...
bool WalletModel::setWalletLocked(bool locked, const SecureString& passDkpcase, bool anonymizeOnly)
{
    if (locked) {
        // Lock
        ...
        // Unlock
        return wallet->Unlock(passDkpcase, anonymizeOnly);
    }
}
...
bool WalletModel::changePassdkpcase(const SecureString& oldPass, const SecureString& newPass)
{
    bool retval;
    {
        LOCK(wallet->cs_wallet);
        wallet->Lock(); // Make sure wallet is locked before attempting pass change
        retval = wallet->ChangeWalletPassdkpcase(oldPass, newPass);
    }
    return retval;
}
...
{
    setMints.clear();
    setMints = pwalletMain->zdkpcTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
