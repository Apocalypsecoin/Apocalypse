...
VERSION=master
commit=false
url=https://github.com/DarkPayCoin/darkpaycoin
proc=2
mem=2000
lxc=true
...
Run this script from the directory containing the darkpaycoin, gitian-builder, gitian.sigs, and darkpaycoin-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://github.com/DarkPayCoin/darkpaycoin
...
then
	# Make output folder
	mkdir -p "./darkpaycoin-binaries/${VERSION}"
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit darkpaycoin=${COMMIT} --url darkpaycoin=${url} ../darkpaycoin/contrib/gitian-descriptors/gitian-linux.yml
	    ...
	    mv build/out/darkpaycoin-*.tar.gz build/out/src/darkpaycoin-*.tar.gz ../darkpaycoin-binaries/${VERSION}
	fi
	# Windows
	if [[ $windows = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit darkpaycoin=${COMMIT} --url darkpaycoin=${url} ../darkpaycoin/contrib/gitian-descriptors/gitian-win.yml
	    ...
	    mv build/out/darkpaycoin-*.zip build/out/darkpaycoin-*.exe ../darkpaycoin-binaries/${VERSION}
	fi
	# Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit darkpaycoin=${COMMIT} --url darkpaycoin=${url} ../darkpaycoin/contrib/gitian-descriptors/gitian-osx.yml
	    ...
	    mv build/out/darkpaycoin-*.tar.gz build/out/darkpaycoin-*.dmg ../darkpaycoin-binaries/${VERSION}
	fi
	# AArch64
	if [[ $aarch64 = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -j ${proc} -m ${mem} --commit darkpaycoin=${COMMIT} --url darkpaycoin=${url} ../darkpaycoin/contrib/gitian-descriptors/gitian-aarch64.yml
...
then
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../darkpaycoin/contrib/gitian-descriptors/gitian-win-signer.yml
	    ...
	    mv build/out/darkpaycoin-*win64-setup.exe ../darkpaycoin-binaries/${VERSION}
	    mv build/out/darkpaycoin-*win32-setup.exe ../darkpaycoin-binaries/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../darkpaycoin/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ...
	    mv build/out/darkpaycoin-osx-signed.dmg ../darkpaycoin-binaries/${VERSION}/darkpaycoin-${VERSION}-osx.dmg
	fi
	popd || exit
